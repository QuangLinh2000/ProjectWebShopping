...
<section class="content-main">
    ...
    <div class="content-header">
        <h2 class="content-title">Lưới Sản Phẩm</h2>
        <div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            ...
            <div id="list-product" class="row gx-3 row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-xl-4 row-cols-xxl-5">
                ...
                <div class="col">
                    <div class="card card-product-grid">
                        <a href="#" class="img-wrap"> <img src="<%=request.getContextPath()%><%=product.getListUrlImg().get(0)%>" alt="Sản phẩm"> </a>
                        <div class="info-wrap">
                            <a href="#" class="title text-truncate"><%=product.getTenSP()%></a>
                            <div class="price mb-2"><span><%=ProductManager.getInstance().formatPrice(product.getGia()-product.getGia()*product.getSell())%>đ</div> <!-- price.// -->

                                <a  class="btn btn-sm btn-light" href="<%=request.getContextPath()%>/admin-detail-product?id=<%=product.getMaSP()%>">
                                    <i class="material-icons md-edit"></i>  Chỉnh sửa
                                </a>


                            <a idsp ="<%=product.getMaSP()%>"  class="btn btn-sm btn-outline-danger delete-product-item"  style="cursor: pointer" >
...
<script>
    ...
    function sendAjax() {
        // ajax jquery
        $.ajax({
            url: "<%=request.getContextPath()%>/admin-product-gird",
            type: "POST",
            data: {
                date: date,
                status: satus,
            ...
            success: function (data) {
                ...
                for (var i = 0; i < listProduct.length; i++) {
                    ...
                    $("#list-product").append('<div class="col">'+
                        '                    <div class="card card-product-grid">'+
                        '                        <a href="#" class="img-wrap"> <img src="<%=request.getContextPath()%>' + product.listUrlImg[0] + '" alt="Sản phẩm"> </a>'+
                        '                        <div class="info-wrap">'+
                        '                            <a href="#" class="title text-truncate">' + product.tenSP + '</a>'+
                        '                            <div class="price mb-2"><span>' + formatPrice(product.gia + product.gia * product.sell) + 'đ</div> <!-- price.// -->'+
                        '                            <a  class="btn btn-sm btn-light" href="<%=request.getContextPath()%>/admin-detail-product?id='+product.maSP+'">'+
                        '                               <i class="material-icons md-edit"></i>  Chỉnh sửa'+
                        '                             </a>'+
                        '                            <a idsp ="'+product.maSP+'"  class="btn btn-sm btn-outline-danger delete-product-item"  style="cursor: pointer" >'+
                        '                                <i class="material-icons md-delete_forever"></i>  Xóa'+
                        '                            </a>'+
                        '                        </div>'+
                        '                    </div> <!-- card-product  end// -->'+
                ...
                if (totalPage != -1){
                    ...
                    for (var i = 1; i <= getPageCount(12,totalPage); i++) {
                        if (i == 1) {
                    ...
                    if(getPageCount(12,totalPage) <= 1) {
                    ...
                    total = getPageCount(12,totalPage);
                    page = 1;
                    clickPage();
                    setNonePageItemChild();
